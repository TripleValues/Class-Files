...
st.markdown(
    ...
    "1987년부터 1989년까지 비행거리 2,500마일을 초과하는 장거리 노선의 수와 "
    "평균 도착지연시간을 연도별로 비교.<br>"
    "노선 수의 증감은 항공사의 장거리 노선 확장 또는 축소 전략을 반영하며, "
    "지연시간의 변화는 운항 효율성 개선 여부를 보여줌.</div>",
    unsafe_allow_html=True,
)
...
st.markdown(
    '<div class="desc-box">📝 <b>분석 포인트</b><br>'
    "비행거리(마일)와 평균 도착지연시간(분) 사이의 상관관계를 산점도로 시각화.<br>"
    "점의 색상은 연도를 구분하며, 거리가 길수록 지연시간이 늘어나는 경향이 있는지 확인할 수 있음.<br>"
    "점의 분포에 따라 특정 연도에 유독 지연이 심한 노선이 존재하는지도 파악할 수 있음.</div>",
    unsafe_allow_html=True,
)
...
st.markdown(
    ...
    "중앙값·사분위 범위·이상치를 함께 확인하여 단순 평균으로는 알 수 없는 "
    "지연 편차와 극단값을 파악.<br>"
